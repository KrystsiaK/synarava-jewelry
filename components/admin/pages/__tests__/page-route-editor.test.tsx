...
import { PageEditRoute, clearAdminPageSaveCache } from "@/components/admin/pages/page-route-editor";
...
describe("PageEditRoute", () => {
  ...
  it("keeps product showcase filled after a full remount with a stale server page", async () => {
    const initial = makePage();
    const saved = makePage({
      updatedAt: new Date("2026-01-03"),
      content: { editProductIds: ["tortoise", "half-moon", "bird-alt", "oak-ring"] },
    ...
    // router.refresh() can remount the client route with a stale RSC payload.
    unmount();
    render(<PageEditRoute page={initial} productOptions={productOptions} translationLocales={[]} />);
