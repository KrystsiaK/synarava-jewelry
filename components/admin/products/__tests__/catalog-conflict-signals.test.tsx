...
import { CatalogConflictStatus, CatalogConflictRowBadges, CatalogConflictListModal } from "@/components/admin/products/catalog-conflict-signals";
...
describe("catalog conflict signals", () => {
  it("offers two equivalent explicit entrances and textual status", () => {
    const open = vi.fn();
    render(<><CatalogConflictStatus signals={signals} onShow={open} /><CatalogConflictStatus signals={signals} onShow={open} compact /></>);
    expect(screen.getAllByText(/2 products with conflicts/)).toHaveLength(2);
  ...
  it("shows all languages in the read-only list and distinguishes failed lookup from zero conflicts", () => {
    const { rerender } = render(<CatalogConflictStatus signals={{ ...signals, state: "failed", totalCount: null, products: {} }} onShow={vi.fn()} />);
    expect(screen.getByText(/status unavailable/i)).toBeTruthy();
    expect(screen.queryByText(/0 products/)).toBeNull();
    rerender(<CatalogConflictListModal open onClose={vi.fn()} signals={signals} products={[{ id: "a", name: "Amber ring", sku: "AR-1" }, { id: "b", name: "Silver chain", sku: "SC-1" }]} focusedProductId="a" />);
    expect(screen.getByRole("dialog", { name: /catalog conflicts/i })).toBeTruthy();
    expect(screen.getByText(/DE · Deutsch · 1 field/)).toBeTruthy();
    expect(screen.getByText(/RU · Русский · 1 field/)).toBeTruthy();
  });
  ...
  it.each([
    ...
    render(<CatalogConflictStatus signals={{ ...signals, state }} onShow={vi.fn()} />);
    expect(screen.getByText(message)).toBeTruthy();
  });

  it("updates both counters when the last persisted conflict disappears", () => {
    const { rerender } = render(<><CatalogConflictStatus signals={signals} onShow={vi.fn()} /><CatalogConflictStatus signals={signals} onShow={vi.fn()} compact /></>);
    ...
    rerender(<><CatalogConflictStatus signals={cleared} onShow={vi.fn()} /><CatalogConflictStatus signals={cleared} onShow={vi.fn()} compact /></>);
    expect(screen.getAllByText("No saved conflicts")).toHaveLength(2);
