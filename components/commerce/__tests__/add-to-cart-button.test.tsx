...
describe("AddToCartButton", () => {
  ...
  it("shows success message after successful add", async () => {
    ...
    render(<AddToCartButton productSlug="birch-bracelet" />);
    await user.click(screen.getByRole("button"));
    await waitFor(() => expect(screen.getByText("Product added to cart")).toBeInTheDocument());
  });
  ...
  it("shows cart count after successful add", async () => {
    ...
    render(<AddToCartButton productSlug="birch-bracelet" />);
    await user.click(screen.getByRole("button"));
    await waitFor(() => expect(screen.getByText("3 products in cart")).toBeInTheDocument());
  });
