...
- полный unit/component suite: 54 файла, 338 тестов; production build собрал 53 маршрута. Локальная БД во время build была выключена, но предусмотренные fallback-пути позволили сборке завершиться успешно.
...
- 30 из 30 публичных браузерных сценариев прошли на fallback-данных, включая отдельный contract test событий commerce-воронки;
...
- отдельный resilience-pass подтвердил recovery из пустой выдачи, блокировку покупки при нулевом остатке, отсутствие horizontal overflow при длинном product title и доступность основных catalog controls при медленных изображениях; карточка и PDP теперь показывают локализованный media fallback при ошибке загрузки;
