...
Use **Check now** to refresh the comparison. For a difference, review both values and choose the version to keep for each field before applying. An entity editor also has **Sync details** / **Review** beside the active language tab; **Compare and decide** opens the scoped comparison in Localization. A failed or incomplete check is not evidence that the values match. Catalog products can also be resolved from **Admin → Products** via **Show conflicts**; that flow previews every field and writes only supported translation fields plus Vendor, Product type, Variant SKU, Price, and Compare-at price. See [`admin/catalog-conflict-resolution-ux.md`](./admin/catalog-conflict-resolution-ux.md).
