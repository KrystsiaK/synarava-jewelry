...
export default async function CheckoutErrorPage({ searchParams }: Props) {
  const [resolvedParams, locale] = await Promise.all([searchParams, getRequestLocale()]);
    ...
    >
      <section className="panel p-8 md:p-10">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <ArtifactLink href={localePath(locale, "/cart")}>
            Return to cart
          </ArtifactLink>
          <ArtifactLink href={localePath(locale, "/checkout/shipping")} variant="secondary">
            Restart shipping
          </ArtifactLink>
        </div>
      </section>
