...
export default async function ConfirmedPage({ searchParams }: Props) {
  ...
  if (isStripeRedirect) {
    ...
    if (params.session_id) {
      ...
      if (!order) redirect(localePath(locale, "/checkout/error?reason=payment"));
    } else {
      ...
      >
        ...
        <section className="panel p-8 md:p-10">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <ArtifactLink href={localePath(locale, "/shop")} size="md">
              Back to shop
            </ArtifactLink>
            <ArtifactLink href={localePath(locale, "/collections")} variant="secondary" size="md">
              Browse collections
            </ArtifactLink>
          </div>
        </section>
