...
export default async function ShippingPage() {
  const [cart, { t, locale }] = await Promise.all([getCartViewModel(), getServerTranslations()]);

  if (cart.itemCount === 0) redirect(localePath(locale, "/cart"));
          ...
          {!user ? (
            <section className="panel p-6">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Link
                  href={`${localePath(locale, "/login")}?redirectTo=${encodeURIComponent(localePath(locale, "/checkout/shipping"))}`}
