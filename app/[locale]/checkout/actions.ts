...
import { getServerTranslations, getRequestLocale } from "@/lib/i18n/server";
...
export async function submitShippingAction(
  ...
  redirect(localePath(await getRequestLocale(), "/checkout/payment"));
}
...
export async function confirmOrderAction() {
  ...
  if (!orderId) {
    redirect(localePath(locale, "/checkout/error?reason=payment"));
  }
  ...
  redirect(localePath(locale, `/checkout/confirmed?order=${orderId}`));
}

export async function resetCheckoutAction() {
  await clearCheckoutOrderCookie();
  redirect(localePath(await getRequestLocale(), "/cart"));
}
