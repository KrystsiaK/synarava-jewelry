...
export default async function PrivacyPage() {
  ...
  if (locale === "pt") {
    return <PortuguesePrivacyPolicy legalName={legalName} postalAddress={postalAddress} privacyEmail={privacyEmail} homeHref={homeHref} offerHref={offerHref} heroImage={heroImage} />;
  }
    ...
    <main className="artifact-shell min-h-screen pb-20 md:pb-32">
      ...
      <header className={cn("relative overflow-hidden border-b border-stroke", heroImage ? "flex min-h-[68svh] items-end py-12 pt-24 md:min-h-[76svh] md:py-16 md:pt-28" : "pt-24 md:pt-28")}>
        ...
        <div className="site-shell relative z-10 w-full pb-10 md:pb-14">
          ...
          <p className={cn("mt-4 max-w-xl text-base leading-7 md:mt-5 md:text-lg md:leading-8", heroImage ? "text-white/75" : "text-foreground/60")}>
      ...
      <div className="site-shell mt-10 grid gap-12 md:mt-14 lg:grid-cols-[16rem_minmax(0,1fr)] lg:gap-16 xl:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        <article className="prose-legal space-y-12 md:space-y-14">
          ...
          <div className="flex flex-wrap gap-4 border-t border-stroke pt-10">
            <Link href={homeHref} className="label-caps text-muted transition-colors hover:text-foreground">← Back to store</Link>
            <Link href={offerHref} className="label-caps text-muted transition-colors hover:text-foreground">Public Offer Agreement →</Link>
          </div>
        </article>
      </div>
    </main>
