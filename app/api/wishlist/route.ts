...
export async function GET(request: Request) {
  const customer = await getShopifyCustomerProfile().catch(() => null);
  if (!customer) return NextResponse.json({ ok: true, isSaved: false });
  ...
  const wishlist = await getShopifyCustomerWishlistIds(customer.id);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await toggleShopifyCustomerWishlist(customer.id, product.shopifyProductId);
