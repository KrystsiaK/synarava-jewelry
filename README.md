...
`read_customers`, `read_orders`, and `read_products` scopes. **Run conflict check** validates the
Shopify connection and scopes, establishes the initial store binding, and registers filtered
...
Edits made in Shopify Translate & Adapt are read back during a product **Refresh from Shopify** or
the catalog conflict check. Shopify does not expose a translation-update webhook, so
...
changed to a duplicated Shopify store, **Run conflict check** reports the mismatch and offers an
explicit rebind action. Rebinding clears only old store-specific IDs; subsequent saves, webhooks,
product refreshes, and conflict checks establish the new store identities. The new store still needs its own app
