...
- [x] Keep conflict actions in a focused modal from the same locale control. `Sync details` always opens the modal; with differences it names only the affected fields and their direction, then offers `Check this language` and `Compare and decide`. The modal deliberately contains no write action: a destructive choice still requires the existing full comparison and impact preview.
