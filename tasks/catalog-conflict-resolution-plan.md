...
### 1. Единый read model конфликта товара — ✅ реализовано (2026-09-22, исправлено 2026-09-22)
...
- Метки `Name`/`Handle`/`Description`/`SEO title`/`SEO description` из commerce-инспекции исключаются в пользу translation-reconcile под локалью `en`, **но только когда у товара уже есть `ShopifyTranslationBinding`** (`findTranslationBinding("PRODUCT", productId)`). Без биндинга translation-reconcile никогда не покроет EN-контент этого товара, поэтому commerce-версия остаётся единственным источником и не отбрасывается.
...
**Исправленная проблема (найдена в код-ревью 2026-09-22):** `getLatestReconcileDifferences()` в `lib/shopify/reconciliation-run.ts` фильтровала расхождения по id **единственного глобально последнего** run'а. После точечной проверки одного товара или одной локали (`ENTITY`/`LOCALE`/`bindingId`-scoped run) этот run становился «последним», и расхождения всех остальных, ранее проверенных и всё ещё нерешённых товаров/локалей исчезали из ответа — индикатор конфликтов и `getProductCatalogConflict` могли молча показать меньше конфликтов, чем есть на самом деле. То же самое ломало `getLatestEntityReconcileState` (используется в `/admin/api/shopify/reconcile`) при вырожденном случае, когда самый последний run вообще проваливался (`FAILED`) — валидные более ранние расхождения переставали быть видны, хотя пункт UX-спеки прямо требует «конфликтов «0» из-за ошибки не бывает».

Исправление — на уровне персистентности, а не в вызывающем коде (один guard в общей функции вместо guard в каждом потребителе):
- `persistDifferences` теперь всегда сначала «гасит» (`resolvedAt = now()`) старые нерешённые строки именно для проверенной пары `(bindingId, locale)`, которые в новом прогоне больше не встретились (значит поле снова совпало), и только затем вставляет свежие строки для того, что всё ещё различается. Раньше при отсутствии новых различий функция просто выходила, не трогая БД — то есть «починенное» поле молча оставалось помеченным конфликтным, пока не наступал единственный «последний» run, который его перекрывал.
- `getLatestReconcileDifferences()` больше не фильтрует по единственному `runId`: она берёт **последнюю нерешённую строку по каждой уникальной паре `(bindingId, locale, fieldKey)`** (`ROW_NUMBER() OVER (PARTITION BY ...)`), независимо от того, какой run её произвёл. Расхождение остаётся видимым, пока эта конкретная пара (binding, locale) не будет перепроверена и не подтвердит совпадение — точечный recheck другого товара/локали больше не может его скрыть.
- `getLatestEntityReconcileState` (в `app/admin/api/shopify/reconcile/route.ts`) обновлена под новую семантику: больше не отбрасывает результат целиком, если самый последний run не `SUCCEEDED`/`PARTIAL` — расхождения теперь самодостаточны и не нуждаются в этом гейте.
- `getProductCatalogConflict`/`listConflictedProductIds` соответственно больше не читают `getLatestReconcileRun()` и не имеют флага `translationChecked` — они всегда включают то, что реально лежит в БД как нерешённое. Статус-баннер каталога («Проверяем Shopify…» / «Повторить проверку», это будет собираться в этапе 3) должен читать `getLatestReconcileRun()` отдельно, напрямую — это чисто информационный сигнал о самой последней попытке, и он не должен решать, какие конфликты показывать.

**Критерии:** ✅ конфликт одного языка не помечает остальные; shared commerce не получает язык; несколько локалей одного товара отображаются отдельно; языки берутся из реестра без фиксированного списка (проверено на 4-й локали DE); отсутствие ещё не созданного перевода не выдается за конфликт; точечная/неудачная проверка не прячет ранее найденные, всё ещё актуальные конфликты других товаров/локалей; EN-конфликт не исчезает бесследно, если товар ещё не привязан к translation-reconcile.

**Проверка:** `lib/shopify/__tests__/catalog-conflict.test.ts` — 15 table-driven тестов (shared/EN/PT/RU/DE, мультиязычный товар, чужой productId, LOCAL_ONLY/SHOPIFY_ONLY, commerce-фоллбек без биндинга, «старый run всё равно виден»). `lib/shopify/__tests__/reconciliation-run.test.ts` — новый тест на retire-then-insert SQL для `persistDifferences`. `npx tsc --noEmit` чист; полный `lib/shopify` + `app/admin` тест-сьют (225 тестов) зелёный.

**Зависимости:** нет. **Места:** `lib/shopify/catalog-conflict.ts` (новый), `lib/shopify/__tests__/catalog-conflict.test.ts` (новый), `lib/shopify/reconciliation-run.ts` (`persistDifferences`, `getLatestReconcileDifferences`, `getLatestEntityReconcileState`), `lib/shopify/__tests__/reconciliation-run.test.ts`.
