...
### 2. Контракт preview/apply и защита записи — ✅ реализовано (2026-09-22, исправлено 2026-09-22)
...
**Критерии:** ✅ изменение после preview → `STALE` без перезаписи (fingerprint mismatch **и** поле, вовсе исчезнувшее из конфликта, оба покрыты); partial success виден по каждой записи отдельно; повторный batch идемпотентен (доказано тестом); очистка непустого значения требует `acknowledgeClears`; commerce-поле никогда не пишется через этот contract — всегда `UNSUPPORTED`, с объяснением, без побочной записи соседних полей.

**Проверка:** `lib/shopify/__tests__/catalog-conflict-apply.test.ts` — 15 server contract тестов (BULK/PRODUCT/MANUAL resolve, commerce всегда в `excluded`/`UNSUPPORTED` и без вызова `applyReconcileChoice`, STALE-поле рядом с одобренным translation-полем разрешается независимо, clearing gate до/после подтверждения, stale по fingerprint и по полностью исчезнувшему полю, partial success на разных товарах, идемпотентный повторный submit, отказ на batch больше лимита). `lib/shopify/__tests__/catalog-conflict.test.ts` — доп. 2 теста на новые `sourceId`/commerce-fingerprints (эти поля используются read model'ом и остаются проверены, даже если commerce-apply сейчас не пишет). `npx tsc --noEmit`, `eslint` на изменённые файлы и полный `lib/shopify` + `app/admin` тест-сьют (244 теста) чисты.
...
- Этапы 3–6 (UI) пока не вызывают новые экшены — потребитель появится вместе с диалогами каталога.
- **Commerce-scoped apply остаётся нерешённой задачей**, а не просто недоделанной: нужен либо честный per-field commerce diff+writer (переписать `inspectProductSyncState`/push/pull вокруг конкретных Shopify-мутаций на поле, а не whole-product query), либо единая read model для commerce и translation, построенная на одном и том же источнике различий — сейчас это две независимые системы обнаружения конфликтов. До этого Диалог 1's per-product/bulk commerce-действия продолжают идти через существующие `pushSingleProductToShopifyAction`/`pullSingleProductFromShopifyAction`, не через `catalog-conflict-apply.ts`.
- Перед этапом 7 (выпуск) — обязательный live Shopify round trip для translation apply (`applyReconcileChoice`) на реальном тестовом товаре.
